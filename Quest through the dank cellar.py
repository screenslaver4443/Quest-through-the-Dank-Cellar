...
print("Hey you! \nYou're finally awake. \nWe need some help dealing with this messed up cellar we got over there.\nThe entrance is just ahead, yeah down that trapdoor.")
choice = 0
while choice != "Y" or "N" :  
    choice = input("Enter the cellar (Y/N)")
    if choice == "Y" :
        break
    elif choice == "N" :
        print("ok lol bye")
...
while True:
    ...
    if room == 3:
    	proom=3
    	monster = random.choice(normalmonsters)
    	print(f"You enter a room covered in weird neon colours, and with weed, the numbers 69 and 420, the American flag with a gun overlayed on top of it, a camera being held by mum.\nWait is that a {monster}, get ready for a fight.")
    	fight()
    	roomstraveled =+ 1
    if room == 4:
    	proom =4
